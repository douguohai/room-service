...
func HandleConnected(s socketIo.Conn) error {
	...
	if len(queryArr) != 0 {
		...
		if ok {
			...
			//判断当前是否已经存在长连接，若存在，关闭原来的socket，以新的为主
			if coon, ok := userToken2SocketIdMap.Get(userToken); ok {
				coon2 := coon.(socketIo.Conn)
				_ = coon2.Close()
			}
...
func HandDisconnected(s socketIo.Conn, reason string) {
	...
	if ok2 {
		go cleanEmptyRoom(user.(entity.User))
	}
}

//异常处理逻辑
func handError(s socketIo.Conn, result entity.Result) {
	log.Printf(result.ToString())
	s.Emit("errored", result)
}

// HandPing 异常处理逻辑
func HandPing(s socketIo.Conn) {
